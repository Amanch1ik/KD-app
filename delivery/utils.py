...
from .models import DeliveryPerson, Order, Restaurant, DeviceToken
...
def send_push_notification(user_or_tokens, title, message):
    ...
    if isinstance(user_or_tokens, list):
        ...
        registration_ids = [dt.registration_id for dt in user_or_tokens.device_tokens.all()]
    elif hasattr(user_or_tokens, 'user') and hasattr(user_or_tokens.user, 'device_tokens'):
        # Если это DeliveryPerson, получаем токены через связанного User
        registration_ids = [dt.registration_id for dt in user_or_tokens.user.device_tokens.all()]
    
    if not registration_ids:
        print(f"No device tokens found for {user_or_tokens}. Not sending push notification.")
        return
    ...
    print(f"[PUSH NOTIFICATION MOCK] Sending to {len(registration_ids)} devices. Title: {title}, Message: {message}")
